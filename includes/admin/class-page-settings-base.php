 ...
 * @since 1.0.0
 */
abstract class WPCV_CiviCRM_Mattermost_Page_Settings_Base {
	...
	protected function admin_help_get() {
		...
		if ( ! file_exists( $template ) ) {
			$template = WPCVMM_PATH . $this->path_template . $this->path_help . 'page-settings-help.php';
		}
	...
	public function page_render() {
		...
		if ( ! file_exists( $template ) ) {
			$template = WPCVMM_PATH . $this->path_template . $this->path_page . $template_name;
		}
	...
	public function page_tab_render() {
		...
		// Check if the Settings Page Tab should be rendered.
		if ( ! $this->page_tab_render_allow() ) {
			return;
		}
	 ...
	 *
	 * Return boolean false in a class that extends this to prevent rendering.
	 *
	 * @since 1.0.0
	 *
	...
	public function meta_box_submit_render( $unused, $metabox ) {
		...
		if ( ! file_exists( $template ) ) {
			$template = WPCVMM_PATH . $this->path_template . $this->path_metabox . 'metabox-settings-submit.php';
		}
